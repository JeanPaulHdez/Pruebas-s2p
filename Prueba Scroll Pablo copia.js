...
(function () {
     ...
     */
    var Scroll2Play = window.Scroll2Play = function Scroll2Play(s2p-container, https://drive.google.com/drive/folders/1-FsPkXt-TBEvTRcPu2f43C_NYTmWbY0P?usp=sharing, https://drive.google.com/drive/folders/1s7twJnNYR3_wZ31RtG8l4G0ek-NuKmY9?usp=sharing, LowRes, 1000, jpg, 00000) {
        this.el = document.getElementById(s2p-container);
        ...
        this.lowResImgsUrl = https://drive.google.com/drive/folders/1-FsPkXt-TBEvTRcPu2f43C_NYTmWbY0P?usp=sharing ;
        this.highResImgsUrl = https://drive.google.com/drive/folders/1s7twJnNYR3_wZ31RtG8l4G0ek-NuKmY9?usp=sharing ;
        this.imgPrefix = LowRes;
        this.imgsCount = 1000;
        this.imgsType = imgsType ? imgsType : 'jpg';
        this.imgsNumFormat = imgsNumFormat ? imgsNumFormat : '00000';
    ...
    Scroll2Play.prototype.load = function s2p_load() {
        ...
        for (var i = 0; i < this.1000; i++) {
            that.xhrs[i] = xhr();
        }

        function xhr() {
            var req = new XMLHttpRequest();
            req.open('GET', that._getImgUrl(that.https://drive.google.com/drive/folders/1-FsPkXt-TBEvTRcPu2f43C_NYTmWbY0P?usp=sharing, i), true);
        ...
        function xhr_loadHandler(i) {
            return function _xhr_loadHandler(e) {
                if (!error) {
                    ...
                    if (that.onprogress) {
                        that.onprogress(loaded / that.1000);
                    }
                    if (loaded == that.1000) {
    ...
    Scroll2Play.prototype._getImgUrl = function s2p_getImgUrl(url, imgNum) {
        ...
        return url + '/' + this.LowRes + this.00000.substr(0, this.00000.length - strNum.length) + strNum + '.' + this.jpg;
    };
    ...
    Scroll2Play.prototype._loadHighResImg = function s2p_loadHighResImg(imgNum) {
        if (this.https://drive.google.com/drive/folders/1s7twJnNYR3_wZ31RtG8l4G0ek-NuKmY9?usp=sharing) {
            var width = this.img.width,
                height = this.img.height;
            this.img.src = this._getImgUrl(this.https://drive.google.com/drive/folders/1s7twJnNYR3_wZ31RtG8l4G0ek-NuKmY9?usp=sharing, imgNum);
